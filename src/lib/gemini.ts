...
{"country":"国家名(完整,如'中国')","region":"一级行政区(中国填省/自治区,如'四川省';美国填州,如'加利福尼亚州';日本填都道府县,如'京都府';法国填大区,如'法兰西岛大区';其他国家填对应的州/省/大区)","city":"景点所在城市,中文通用名(国内保留'市'/'县'等后缀,如'稻城县';国外用通用译名,如'巴黎'/'洛杉矶')","attraction":"景点名","confidence":"high/medium/low","evidence":"推断依据（30字内）","lat":纬度数字,"lng":经度数字,"opening_note":"三行出行参考,每行用\\n分隔。若无法确定具体信息则填null。"}
...
5. region 与 city **不论哪个国家都必须尽量填写**,确实无法判断才填 null。国外景点同样要给出 region(州/省/大区) 与 city(城市),不能因为不是中国就留空
6. **地名后缀**:中国地名必须保留完整行政后缀("市"/"县"/"州"/"地区"/"自治区"等),例如'阿坝藏族羌族自治州'不能简写成'阿坝','稻城县'不能写成'稻城';**国外地名用中文通用译名,不要强行加"市"后缀**(如写'巴黎'而非'巴黎市','洛杉矶'而非'洛杉矶市')
