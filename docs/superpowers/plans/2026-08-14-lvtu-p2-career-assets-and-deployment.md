...
短版控制为 2 至 3 条，每条使用“动作 + 产品判断 + 可验证证据”；长版补充模型字段、人工确认与 Badcase。角色必须如实说明：用户提出痛点与基本功能，在对话中选择方向、批准规格和结果；AI 代理承担方案细化、代码、公开样本与标注、模型调用、测试、复测和文档草稿。
...
脚本逐份扫描案例页和三份求职材料，从评测报告、运行记录与 Badcase 数据派生模型名和各类数字，验证每份材料的全部同类声明一致，并锁定实时、回放与协作边界。
